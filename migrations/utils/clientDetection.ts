 ...
 */
export function shouldSkipForPostgreSQL(knex: Knex, migrationName: string): boolean {
  const client = knex.client.config?.client
  if (client === POSTGRESQL_CLIENT) {
    console.log(`Skipping migration ${migrationName} - PostgreSQL uses consolidated schema in migration 034`)
    return true
  }
  return false
}
