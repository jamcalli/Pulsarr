...
export async function up(knex: Knex): Promise<void> {
  ...
  `)
    ...
    await knex.schema.createTable('sonarr_instances', (table) => {
      ...
      table
        .specificType('monitor_new_items', 'monitor_new_items')
        .defaultTo('all')
    ...
    await knex.schema.createTable('radarr_instances', (table) => {
      ...
      table.boolean('search_on_add').defaultTo(true)
      table
        .specificType('minimum_availability', 'minimum_availability')
        .defaultTo('released')
      table.jsonb('tags').defaultTo('[]')
      table.boolean('is_default').defaultTo(false)
      table.boolean('is_enabled').defaultTo(true)
    ...
    await knex.schema.createTable('configs', (table) => {
      ...
      table
        .specificType('deleteSyncNotify', 'delete_sync_notify')
        .defaultTo('none')
      ...
      table
        .specificType('removedTagMode', 'removed_tag_mode')
        .defaultTo('remove')
    ...
    await knex.schema.createTable('watchlist_items', (table) => {
      ...
      table
        .specificType('status', 'watchlist_status')
        .notNullable()
        .defaultTo('pending')
