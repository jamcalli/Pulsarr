...
export class PlexWatchlistService {
  ...
  private extractKeysAndRelationships(userWatchlistMap: Map<Friend, Set<TokenWatchlistItem>>) {
    ...
    this.log.info(`Collected ${userKeyMap.size} users and ${allKeys.size} unique keys`);
    return { allKeys, userKeyMap };
  }

  private getExistingItems(userKeyMap: Map<string, Set<string>>, allKeys: Set<string>) {
    const allUsers = Array.from(userKeyMap.keys());
    const existingItems = this.db.getBulkWatchlistItems(allUsers, Array.from(allKeys));
  ...
  private async processAndSaveNewItems(brandNewItems: Map<Friend, Set<TokenWatchlistItem>>) {
    ...
    if (processedItems instanceof Map) {
      ...
      if (itemsToInsert.length > 0) {
        ...
        this.log.info(`Processed and inserted ${itemsToInsert.length} new items`);
      }
  
      return processedItems;
    } else {
      throw new Error('Expected Map<Friend, Set<WatchlistItem>> from processWatchlistItems');
    }
  }

  private linkExistingItems(existingItemsToLink: Map<Friend, Set<WatchlistItem>>) {
  ...
  private buildResponse(
    ...
    return {
      total: this.calculateTotal(existingItems, existingItemsToLink, processedItems),
  ...
  private prepareItemsForInsertion(processedItems: Map<Friend, Set<WatchlistItem>>) {
  ...
  private calculateTotal(
    ...
    const linkItemsCount = Array.from(existingItemsToLink.values())
      .reduce((acc, items) => acc + items.size, 0);
    const processedItemsCount = Array.from(processedItems.values())
      .reduce((acc, items) => acc + items.size, 0);
  ...
  private formatLinkedItems(existingItemsToLink: Map<Friend, Set<WatchlistItem>>, user: Friend) {
    return existingItemsToLink.has(user) 
      ? Array.from(existingItemsToLink.get(user)!).map(this.formatWatchlistItem)
      : [];
  }

  private formatProcessedItems(processedItems: Map<Friend, Set<WatchlistItem>>, user: Friend) {
