...
interface DatabaseOperations {
    ...
    updateUser(id: number, data: Partial<Omit<User, 'id' | 'created_at'>>): boolean;
...
class DatabaseConnection implements DatabaseOperations {
    ...
    private statements: {
        ...
        createConfig: Statement<[string, number, string | undefined, string | undefined]>;
        updateConfig: Statement<[string | undefined, number | undefined, string | undefined, string | undefined, number]>;
        getWatchlistItem: Statement<[string, string]>;
        createWatchlistItem: Statement<[string, string, string, string, string, string]>;
    ...
    private constructor(private logger: FastifyBaseLogger) {
        const verboseLogger = (message?: unknown, ...additionalArgs: unknown[]) => {
            if (message && typeof message === 'string' && message.startsWith('Error:')) {
        ...
        this.statements = {
            ...
            createUser: this.db.prepare('INSERT INTO users (name, email) VALUES (?, ?)'),
            updateUser: this.db.prepare('UPDATE users SET name = COALESCE(?, name), email = COALESCE(?, email) WHERE id = ?'),
    ...
    public updateUser(id: number, data: Partial<Omit<User, 'id' | 'created_at'>>): boolean {
        const result = this.statements.updateUser.run(data.username, data.email, id);
        return result.changes > 0;
    }

    public getConfig(id: number): Config | undefined {
        const row = this.statements.getConfig.get(id) as { plexTokens: string, port: number, selfRss?: string, friendsRss?: string } | undefined;
        if (row) {
            return {
    ...
    public getWatchlistItem(user: string, key: string): WatchlistItem | undefined {
        return this.statements.getWatchlistItem.get(user, key) as WatchlistItem | undefined;
    }
...
export const getDbInstance = (logger: FastifyBaseLogger): DatabaseConnection => {
