...
/**
 * Configures the Fastify server with plugin autoloading, Vite-powered SPA integration, global error and 404 handling, and authentication-aware routing with conditional redirection.
 *
 * Loads middleware for form body parsing, registers external and custom plugins, and sets up route handlers. Integrates Vite for serving a single-page application. Implements global error and not-found handlers with logging and rate limiting. Defines root and app routes that manage user sessions, authentication bypass, and redirects based on user existence and Plex token configuration.
 *
 * @remark
