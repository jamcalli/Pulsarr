...
export class UserTagService {
    ...
    try {
      if (emitProgress) {
        this.emitProgress({
          ...
          phase: 'start',
          progress: 0,
          message: 'Starting complete user tag synchronization...',
        })
      }
      ...
      if (emitProgress) {
        this.emitProgress({
          ...
          phase: 'sonarr-sync',
          progress: 5,
          message: 'Syncing Sonarr tags...',
        })
      }

      const sonarrResults = await this.syncSonarrTags()
      ...
      if (emitProgress) {
        this.emitProgress({
          ...
          phase: 'radarr-sync',
          progress: 40,
          message: 'Syncing Radarr tags...',
        })
      }

      const radarrResults = await this.syncRadarrTags()
    ...
    try {
      if (emitProgress) {
        this.emitProgress({
          ...
          phase: 'start',
          progress: 0,
          message: 'Starting orphaned user tag cleanup...',
        })
      }
      ...
      if (emitProgress) {
        this.emitProgress({
          ...
          progress: 5,
          message: 'Processing Radarr instances for orphaned tags...',
        })
      }
      ...
      for (const instance of radarrInstances) {
        try {
          ...
          for (let i = 0; i < Array.from(movies).length; i += BATCH_SIZE) {
            ...
            // Update counts
            for (const result of batchResults) {
              if (result.removed) results.radarr.removed++
              if (result.skipped) results.radarr.skipped++
              if (result.failed) results.radarr.failed++
            }
      ...
      if (emitProgress) {
        this.emitProgress({
          ...
          progress: 50,
          message: 'Processing Sonarr instances for orphaned tags...',
        })
      }
      ...
      for (const instance of sonarrInstances) {
        try {
          ...
          for (let i = 0; i < Array.from(allSeries).length; i += BATCH_SIZE) {
            ...
            // Update counts
            for (const result of batchResults) {
              if (result.removed) results.sonarr.removed++
              if (result.skipped) results.sonarr.skipped++
              if (result.failed) results.sonarr.failed++
            }
  ...
  /**
   * Remove all user tags from all media items in both Sonarr and Radarr with batching and progress tracking
    ...
    try {
      if (emitProgress) {
        this.emitProgress({
          ...
          phase: 'start',
          progress: 0,
          message: `Starting complete user tag removal${deleteTagDefinitions ? ' with tag definition deletion' : ''}...`,
        })
      }

      // Remove Sonarr tags first
      if (emitProgress) {
        this.emitProgress({
          operationId,
          type: 'tag-removal',
          phase: 'sonarr-removal',
          progress: 5,
          message: 'Removing Sonarr user tags...',
        })
      }

      const sonarrResults =
        await this.removeAllSonarrUserTags(deleteTagDefinitions)

      // Then remove Radarr tags
      if (emitProgress) {
        this.emitProgress({
          operationId,
          type: 'tag-removal',
          phase: 'radarr-removal',
          progress: 50,
          message: 'Removing Radarr user tags...',
        })
      }

      const radarrResults =
        await this.removeAllRadarrUserTags(deleteTagDefinitions)
