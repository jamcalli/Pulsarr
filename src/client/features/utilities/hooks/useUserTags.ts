...
/**
 * Checks if the provided action result represents a create tag operation.
 *
...
/**
 * Checks if the given action result is a cleanup tag response.
 *
 * Returns true if the response has a `radarr` property with a nested `removed` field and does not have a `mode` property.
 */
...
/**
 * React hook for managing user tagging configuration and related actions for Sonarr and Radarr.
 *
 * Provides form state and validation for user tagging settings, integrates with external stores for configuration and operation state, and exposes handler functions for creating, syncing, cleaning up, and removing user tags. Also manages UI state for tag deletion confirmation and tracks the status of tag deletion operations.
 *
 * @returns An object containing the form instance, loading and saving flags, error state, results of the latest actions, tag deletion status, and handler functions for all user tag management operations.
 */
