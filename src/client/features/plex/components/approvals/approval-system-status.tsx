 ...
 */
export function ApprovalSystemStatus({
  job,
  formatLastRun,
  formatNextRun,
  isLoading,
}: ApprovalSystemStatusProps) {
  if (isLoading) {
    return (
      <div>
      ...
      {job && (
        <div className="mt-4 p-3 bg-muted/50 rounded-md">
          <p className="text-sm text-muted-foreground">
            <strong>What this schedule does:</strong> Expires old approval
            requests based on configuration, automatically approves expired
            requests if enabled, and cleans up old expired requests to maintain
