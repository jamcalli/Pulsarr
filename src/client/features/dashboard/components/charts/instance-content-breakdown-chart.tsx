...
export default function InstanceContentBreakdownChart() {
  ...
  const cssColors = {
    grabbed: getComputedStyle(document.documentElement)
      .getPropertyValue('--chart-1')
      .trim() || '196 39% 33%',
    notified: getComputedStyle(document.documentElement)
      .getPropertyValue('--chart-3')
      .trim() || '29 85% 87%',
    requested: getComputedStyle(document.documentElement)
  ...
  const chartData = useMemo(() => {
    ...
    return instanceContentBreakdown.map((instance) => {
      ...
      instance.by_status.forEach((status) => {
  ...
  if (isLoading || !instanceContentBreakdown || instanceContentBreakdown.length === 0) {
    return (
          ...
          <div className="flex-grow w-full md:min-h-0 min-h-[350px]">
            <div className="h-full w-full flex items-center justify-center">
              <span className="text-text text-muted-foreground">
                {isLoading ? 'Loading instance data...' : 'No instance data available'}
              </span>
            </div>
          </div>
          </CardContent>
        ...
        <CardContent className="pt-4 flex-grow flex flex-col">
          <div className="flex-grow w-full md:min-h-0 min-h-[350px]">
            ...
            >
              ...
              <Bar dataKey="grabbed" stackId="a" name="Grabbed" fill={`hsl(${cssColors.grabbed})`} />
              <Bar dataKey="notified" stackId="a" name="Notified" fill={`hsl(${cssColors.notified})`} />
              <Bar dataKey="requested" stackId="a" name="Requested" fill={`hsl(${cssColors.requested})`} />
            </BarChart>
          </ChartContainer>
          </div>
          ...
          <div className="flex flex-wrap justify-center mt-3 gap-3 flex-shrink-0">
            <div className="flex items-center">
              <span className="h-3 w-3 rounded-full inline-block mr-2" style={{ backgroundColor: `hsl(${cssColors.grabbed})` }} />
              <span className="text-sm text-text">Grabbed</span>
            </div>
            <div className="flex items-center">
              <span className="h-3 w-3 rounded-full inline-block mr-2" style={{ backgroundColor: `hsl(${cssColors.notified})` }} />
              <span className="text-sm text-text">Notified</span>
            </div>
            <div className="flex items-center">
              <span className="h-3 w-3 rounded-full inline-block mr-2" style={{ backgroundColor: `hsl(${cssColors.requested})` }} />
