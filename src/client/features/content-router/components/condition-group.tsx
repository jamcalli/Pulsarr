...
const ConditionGroupComponent = ({
  ...
  // Create an empty condition with the most common fields
    ...
    <div className={`border-l-2 pl-4 ${getLevelColor()}`}>
      ...
      <div className="space-y-4">
        {value?.conditions?.map((condition, index) => {
          // Create a unique key based on condition properties
          const conditionKey = 'operator' in condition
            ? `group-${(condition as IConditionGroup).operator}-${index}`
            : `condition-${(condition as ICondition).field}-${(condition as ICondition).operator}-${index}`
