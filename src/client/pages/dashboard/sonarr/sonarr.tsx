...
export default function SonarrConfigPage() {
  ...
  useEffect(() => {
    const initializeComponent = async () => {
      ...
      form.reset({
        ...
        seasonMonitoring: (firstInstance.seasonMonitoring as SonarrMonitoringType) || 'all',
    ...
    try {
      ...
      if (result.success) {
        ...
        if (selectedInstance) {
          ...
          const instanceResponse = await fetch(`/v1/sonarr/instances/${selectedInstance}`, {
            method: 'PUT',
  ...
  const onSubmit = async (data: SonarrInstanceSchema) => {
    ...
    try {
      if (selectedInstance) {
        const response = await fetch(`/v1/sonarr/instances/${selectedInstance}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            ...data,
  ...
  form.watch(['name', 'baseUrl', 'apiKey', 'qualityProfile', 'rootFolder', 'seasonMonitoring']);
