...
export function useSonarrConnection(instance: SonarrInstance, setShowInstanceCard?: (show: boolean) => void) {
  ...
  const testConnectionWithoutLoading = useCallback(async (baseUrl: string, apiKey: string) => {
    ...
    const [response] = await Promise.all([
      fetch(`/v1/sonarr/test-connection?baseUrl=${encodeURIComponent(baseUrl)}&apiKey=${encodeURIComponent(apiKey)}`),
      minimumLoadingTime,
    ])
    if (!response.ok) {
  ...
  const testConnection = useCallback(async (values: SonarrConnectionValues, form: any) => {
    if (!values.name?.trim()) {
      toast({
        title: 'Name Required',
