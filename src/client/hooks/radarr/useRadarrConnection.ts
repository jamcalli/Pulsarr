...
export function useRadarrConnection(instance: RadarrInstance, setShowInstanceCard?: (show: boolean) => void) {
  ...
  const testConnectionWithoutLoading = useCallback(async (baseUrl: string, apiKey: string) => {
    const minimumLoadingTime = new Promise((resolve) => setTimeout(resolve, 500))
    const [response] = await Promise.all([
      fetch(`/v1/radarr/test-connection?baseUrl=${encodeURIComponent(baseUrl)}&apiKey=${encodeURIComponent(apiKey)}`),
      minimumLoadingTime,
    ])
    if (!response.ok) {
  ...
  const testConnection = useCallback(async (values: RadarrConnectionValues, form: any) => {
    if (!values.name?.trim()) {
      toast({
        title: 'Name Required',
    ...
    try {
      ...
      await Promise.all([
        (async () => {
          const testResult = await testConnectionWithoutLoading(values.baseUrl, values.apiKey)
          if (!testResult.success) {
            throw new Error(testResult.message || 'Failed to connect to Radarr')
          }

          const isOnlyPlaceholderInstance = instances.length === 1 && instances[0].apiKey === 'placeholder'
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Failed to connect to Radarr',
        variant: 'destructive',
      })
    }
  }, [
