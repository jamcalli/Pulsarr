...
export class ShowStatusService {
    ...
    private processStatusUpdates(
      ...
      private findSonarrMatch(sonarrItems: Item[], itemGuids: string[] | string | undefined): Item | undefined {
        if (!itemGuids) return undefined;
        
        const guids = Array.isArray(itemGuids) ? itemGuids : 
                      typeof itemGuids === 'string' ? JSON.parse(itemGuids) : [];
