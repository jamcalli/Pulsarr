declare module '../../database.service.js' {
  interface DatabaseService {
     ...
     */
    getMostWatchlistedShows(limit?: number): Promise<{ title: string; count: number; thumb: string | null }[]>
     ...
     */
    getMostWatchlistedMovies(limit?: number): Promise<{ title: string; count: number; thumb: string | null }[]>
     ...
     */
    getUsersWithMostWatchlistItems(limit?: number): Promise<{ name: string; count: number }[]>

    /**
     * Gets the distribution of watchlist item statuses
     * @returns Promise resolving to array of statuses with their counts
     */
    getWatchlistStatusDistribution(): Promise<{ status: string; count: number }[]>
     ...
     */
    getRecentActivityStats(days?: number): Promise<{ new_watchlist_items: number, status_changes: number, notifications_sent: number }>

    /**
     * Gets activity statistics for Sonarr/Radarr instances
     * @returns Promise resolving to array of instance activity statistics
     */
    getInstanceActivityStats(): Promise<Array<{ instance_id: number, instance_type: 'sonarr' | 'radarr', name: string, item_count: number }>>
     ...
     */
    getAverageTimeFromGrabbedToNotified(): Promise<Array<{ content_type: string, avg_days: number, min_days: number, max_days: number }>>

    /**
     * Gets detailed metrics on status transitions
     * @returns Promise resolving to array of detailed status transition metrics
     */
    getDetailedStatusTransitionMetrics(): Promise<Array<{ from_status: string, to_status: string, content_type: string, avg_days: number }>>
     ...
     */
    getAverageTimeToAvailability(): Promise<Array<{ content_type: string, avg_days: number, min_days: number, max_days: number }>>

    /**
     * Gets status flow data for visualization
     * @returns Promise resolving to array of status flow data points
     */
    getStatusFlowData(): Promise<Array<{ from_status: string, to_status: string, content_type: string, count: number }>>
  }
}